...
void writeFile(const std::string & text, const std::string & filename)
{
	...
	std::cout << "[+] The encrypted text was written to " << filename.c_str() << std::endl;
}
...
int decrypt(const std::string & openedText, uint32_t key, std::string & cipherText)
{
	...
	if (residualLength != 0) {
		...
		result = decryptBlock(block | auxiliaryBlock, key);
